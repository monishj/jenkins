    ...
    stages {
        ...
        steps {
          dir("$SERVICE") {
            script {
              ...
              docker = new Docker(this, "$CURRENT_ENVIRONMENT-$IMAGE_TAG", config.serviceName, config.repositoryName)
        ...
        steps {
          dir("$SERVICE") {
            script {
              ...
              artifactory.upload("pact-version.txt", "maximus-general/services-build-versions/$SERVICE/pact-version.txt")
            }
          }
        }
      }
      ...
      stage("Push events") {
        steps {
          dir("$SERVICE") {
            script {
              artifactory.pushEvent(SERVICE, BUILD_NUMBER)
            }
          }
        }
      }
      ...
      stage("Update Helm chart") {
        steps {
          dir("$SERVICE") {
            script {
              sh "sed -ri \"s/tag: latest/tag: ${IMAGE_TAG}/\" helm/${SERVICE}/values.yaml"
              sh "helm lint helm/$SERVICE"
              path = "helm/$SERVICE"
              helm.packageChart("$path", "$IMAGE_TAG")
              artifactory.upload("$SERVICE-${IMAGE_TAG}.tgz", "maximus-helm-local/$config.helm_folder/$SERVICE/$SERVICE-${IMAGE_TAG}.tgz")
            }
          }
        }
      }
      ...
      stage("Print Built Image and Helm Tag") {
        ...
        steps {
          script {
            ...
            sh "cat version.txt"
            artifactory.upload("version.txt", "maximus-general/services-build-versions/$SERVICE/version.txt")
          }
        }
      }
      ...
      stage("Generate Version Manifest") {
        when {
          expression {
            return ((config.gitBranch == 'master' || config.gitBranch == null) && config.serviceName != 'vendor-ui-mock')
          }
        }
        steps {
          build wait: false, job: 'maximus-generate-version-manifest'
        }
      }
      ...
      stage("Generate Version Manifest Replica") {
        ...
        steps {
          build wait: false, job: 'maximus-generate-version-manifest-replica'
        }
      }
    
      stage("Trigger Vendor Ui Mock deploy") {
        when {
          expression {
            return config.serviceName == 'vendor-ui-mock'
          }
        }
        steps {
          build wait: false, job: 'vendor-ui-mock-deploy'
        }
      }
      ...
      stage("Trigger downstream pipelines") {
        steps {
          script {
            if (config.serviceName == 'agent-smith-ui' || config.serviceName == 'agent-smith-service') {
              build wait: false, job: 'SIT/sit-deploy-agent-smith'
            }
            if (config.serviceName == 'misc-service') {
              build wait: false, job: 'SIT/misc-sit-deploy'
            }
            if (config.serviceName == 'spring-config-server') {
              build wait: false, job: 'spring-config-server-deploy'
            }
          }
        }
      }
